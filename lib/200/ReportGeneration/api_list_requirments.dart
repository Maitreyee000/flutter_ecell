...
class ApiListRequirement extends StatefulWidget {
  const ApiListRequirement({super.key, this.idData});
  final idData;

  @override
  State<ApiListRequirement> createState() => _ApiListRequirementState();
}

class _ApiListRequirementState extends State<ApiListRequirement> {
  ...
  Future<void> loadInitialData(data) async {
    ...
    details = await apiController.getDataListById(
        context, "nodal_view_requirement_list", payload);
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            VerticalDivider(),
            buildCell("Status", flex: 2, color: Colors.white),
          ],
        ),
      ),
    );
